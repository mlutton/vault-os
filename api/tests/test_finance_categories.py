...
def _make_item(conn, account_id, **over):
    defaults = dict(
        item_id="p1", name="Rent", estimate_cents=-150000, plan_type="Rent", payee=None,
...
def _make_txn(conn, account_id, **over):
    defaults = dict(
        txn_id="t1", date="2026-03-05", merchant_raw="X", merchant="X", amount_cents=-1000,
        category=None, category_source=None, plan_item_id=None, match_source=None,
        excluded_from_charts=0, dedupe_hash="h1",
    )
    defaults.update(over)
    conn.execute(
        ...
        (
            ...
            defaults["amount_cents"], defaults["category"], defaults["category_source"], defaults["plan_item_id"],
            defaults["match_source"], defaults["excluded_from_charts"], defaults["dedupe_hash"],
        ),
    )
    conn.commit()
...
def test_rows_are_the_union_of_planned_and_actual_key_sets(conn, account_id):
    ...
    _make_txn(conn, account_id, txn_id="t1", category="Rent", amount_cents=-150000, dedupe_hash="h1")
...
def test_catch_all_plan_item_never_contributes_a_planned_row(conn, account_id):
    _make_item(
        conn, account_id, item_id="catch", name="Everything else", plan_type="Other",
        estimate_cents=-5000, kind="budget", reset_period="monthly", cadence="budget",
        day_of_month=None, cadence_unit=None, cadence_frequency=None, is_catch_all=True,
    )
    result = categories.build_categories(conn, TODAY_PERIOD, TODAY)
    assert result["rows"] == []
...
def test_a_dated_plan_item_only_contributes_planned_in_periods_it_actually_occurs(conn, account_id):
    _make_item(
        conn, account_id, plan_type="Insurance", cadence_frequency=3, day_of_month=1,
        anchor_period="2026-01", estimate_cents=-30000,
    )
    result_march = categories.build_categories(conn, "2026-03", TODAY)  # not a quarterly month from a Jan anchor
    result_april = categories.build_categories(conn, "2026-04", TODAY)  # 3 months after Jan -> occurs
...
def test_a_spread_item_contributes_planned_every_period(conn, account_id):
    _make_item(
        conn, account_id, plan_type="Food", kind="budget", reset_period="monthly", cadence="budget",
...
def test_a_budgets_planned_cents_reflects_its_active_adjustment(conn, account_id):
    ...
    store.set_adjusted(conn, "p1", TODAY_PERIOD, -9300, "2026-03-01", "2026-03-11T00:00:00Z", open_period=TODAY_PERIOD)
    result = categories.build_categories(conn, TODAY_PERIOD, TODAY)
...
def test_a_budget_ignores_a_stale_adjustment_from_a_previous_window(conn, account_id):
    ...
    store.set_adjusted(conn, "p1", "2026-02", -5000, "2026-02-01", "2026-02-11T00:00:00Z", open_period=TODAY_PERIOD)
    result = categories.build_categories(conn, TODAY_PERIOD, TODAY)
...
def test_a_weekly_budget_sums_its_per_week_contributions_within_the_period(conn, account_id):
    ...
    _make_item(
        conn, account_id, plan_type="Food", kind="budget", reset_period="weekly", cadence="budget",
...
def test_a_week_unit_dated_item_landing_twice_in_a_period_contributes_the_estimate_twice(conn, account_id):
    ...
    _make_item(
        conn, account_id, plan_type="Subscription", cadence_unit="week", cadence_frequency=2,
        day_of_month=None, anchor_period=None, anchor_date="2026-08-07", estimate_cents=-20000,
    )
    result = categories.build_categories(conn, "2026-08", TODAY)
...
def test_top_8_and_everything_else_aggregate(conn, account_id):
    for i in range(9):
        _make_txn(conn, account_id, txn_id=f"t{i}", category=f"Cat{i}", amount_cents=-(1000 * (9 - i)), dedupe_hash=f"h{i}")
    result = categories.build_categories(conn, TODAY_PERIOD, TODAY)
...
def test_unseen_rows_are_never_folded_into_everything_else_or_the_pie(conn, account_id):
    ...
    assert {r["category"] for r in result["rows"][:9]} == {f"Cat{i}" for i in range(8)} | {"Everything else"}


def test_committed_flexible_split_sums_over_every_observed_category_not_just_top_8(conn, account_id):
    for i in range(9):
        _make_txn(conn, account_id, txn_id=f"t{i}", category="Rent" if i == 8 else f"Cat{i}", amount_cents=-(1000 * (9 - i)), dedupe_hash=f"h{i}")
    result = categories.build_categories(conn, TODAY_PERIOD, TODAY)
...
def test_flexible_is_total_minus_committed(conn, account_id):
    _make_txn(conn, account_id, txn_id="t1", category="Rent", amount_cents=-150000, dedupe_hash="h1")
    _make_txn(conn, account_id, txn_id="t2", category="Entertainment", amount_cents=-5000, dedupe_hash="h2")
    result = categories.build_categories(conn, TODAY_PERIOD, TODAY)
...
def test_a_budget_whose_type_matches_a_committed_name_is_flexible_not_committed(conn, account_id):
    ...
    _make_item(
        conn, account_id, plan_type="Insurance", kind="budget", reset_period="monthly",
        cadence="budget", day_of_month=None, cadence_unit=None, cadence_frequency=None,
        estimate_cents=-5000,
    )
...
def test_committed_flexible_totals_are_correct_for_a_mixed_set_of_postings_and_budgets(conn, account_id):
    _make_item(conn, account_id, item_id="p1", plan_type="Rent", estimate_cents=-150000)
    _make_item(
        conn, account_id, item_id="p2", plan_type="Utility", kind="budget", reset_period="monthly",
        cadence="budget", day_of_month=None, cadence_unit=None, cadence_frequency=None,
        estimate_cents=-8000,
    )
    _make_txn(conn, account_id, txn_id="t1", category="Rent", amount_cents=-150000, dedupe_hash="h1")
    _make_txn(conn, account_id, txn_id="t2", category="Utility", amount_cents=-8000, dedupe_hash="h2")
    result = categories.build_categories(conn, TODAY_PERIOD, TODAY)
...
def test_a_committed_posting_keeps_its_own_spend_committed_even_when_a_budget_shares_its_type_name(conn, account_id):
    ...
    _make_item(conn, account_id, item_id="p1", plan_type="Rent", estimate_cents=-150000)
    _make_item(
        conn, account_id, item_id="p2", plan_type="Rent", kind="budget", reset_period="monthly",
        cadence="budget", day_of_month=None, cadence_unit=None, cadence_frequency=None,
        estimate_cents=-8000,
    )
    _make_txn(conn, account_id, txn_id="t1", category="Rent", amount_cents=-150000, plan_item_id="p1", dedupe_hash="h1")
    _make_txn(conn, account_id, txn_id="t2", category="Rent", amount_cents=-8000, plan_item_id="p2", dedupe_hash="h2")
    result = categories.build_categories(conn, TODAY_PERIOD, TODAY)
    row = result["rows"][0]
...
def test_the_catch_alls_type_never_poisons_an_unseen_committed_category(conn, account_id):
    ...
    _make_item(
        conn, account_id, item_id="catch", name="Everything else", plan_type="Insurance",
        estimate_cents=-5000, kind="budget", reset_period="monthly", cadence="budget",
        day_of_month=None, cadence_unit=None, cadence_frequency=None, is_catch_all=True,
    )
    result = categories.build_categories(conn, TODAY_PERIOD, TODAY)
...
def test_an_unseen_committed_posting_stays_committed_even_when_a_budget_shares_its_type_name(conn, account_id):
    ...
    _make_item(conn, account_id, item_id="p1", plan_type="Insurance", estimate_cents=-27500)
    _make_item(
        conn, account_id, item_id="p2", plan_type="Insurance", kind="budget", reset_period="monthly",
        cadence="budget", day_of_month=None, cadence_unit=None, cadence_frequency=None,
        estimate_cents=-5000,
    )
    result = categories.build_categories(conn, TODAY_PERIOD, TODAY)
...
def test_an_unseen_row_reflects_only_what_actually_contributes_this_period_not_store_wide_existence(conn, account_id):
    ...
    _make_item(
        conn, account_id, item_id="p1", plan_type="Insurance", cadence_frequency=3, day_of_month=1,
        anchor_period="2026-01", estimate_cents=-30000,
    )
    _make_item(
        conn, account_id, item_id="p2", plan_type="Insurance", kind="budget", reset_period="monthly",
    ...
    result = categories.build_categories(conn, "2026-03", TODAY)  # not a quarterly month from a Jan anchor
    row = result["rows"][0]
    assert row["category"] == "Insurance"

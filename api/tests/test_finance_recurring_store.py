...
def _make_txn(conn, account_id, **over):
    defaults = dict(
        txn_id="t1", date="2026-03-01", merchant_raw="NETFLIX", merchant="Netflix",
        amount_cents=-1599, category=None, category_source=None, plan_item_id=None,
...
def test_build_recurring_totals_sum_across_multiple_detected_merchants(conn, account_id):
    for i, d in enumerate(["2026-01-15", "2026-02-14", "2026-03-16"]):
        _make_txn(conn, account_id, txn_id=f"n{i}", date=d, merchant="Netflix", amount_cents=-1599, dedupe_hash=f"n{i}")
    for i, d in enumerate(["2026-01-05", "2026-02-04", "2026-03-06"]):
        _make_txn(conn, account_id, txn_id=f"s{i}", date=d, merchant="Spotify", amount_cents=-999, dedupe_hash=f"s{i}")
    result = recurring.build_recurring(conn)

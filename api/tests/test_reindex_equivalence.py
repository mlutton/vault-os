...
def _write_run(tmp_vault, job_id, skill, source, ts_queued, ts_started, ts_completed, status, exit_code, summary, md_path, deliverable_path):
    record = {
        "id": job_id, "skill": skill, "args": {}, "source": source,
        "ts_queued": ts_queued, "ts_started": ts_started, "ts_completed": ts_completed,
        "status": status, "exit_code": exit_code, "summary": summary,
        "md_path": md_path, "log_path": md_path, "deliverable_path": deliverable_path,
    }
...
def test_reindex_output_matches_incrementally_built_database(tmp_vault, tmp_path):
    ...
    store.create_job(incremental_conn, job_id="a", skill="metrics-pull", args={}, source="api", engine="claude", ts_queued="t0")
    store.apply_event(
        incremental_conn, job_id="a", status="running", ts="t1", received_at="t1",
        pid=100, md_path="system/runs/a.md", deliverable_path="inbox/a.md",
    )
    store.apply_event(incremental_conn, job_id="a", status="ok", ts="t2", received_at="t2", exit_code=0, summary="done a")

    store.create_job(incremental_conn, job_id="b", skill="ai-wire", args={}, source="voice", engine="claude", ts_queued="t3")

    store.create_job(incremental_conn, job_id="c", skill="metrics-pull", args={}, source="api", engine="claude", ts_queued="t4")
    store.apply_event(incremental_conn, job_id="c", status="running", ts="t5", received_at="t5", pid=101, md_path="system/runs/c.md")
    store.apply_event(incremental_conn, job_id="c", status="error", ts="t6", received_at="t6", exit_code=1, summary="boom c")
    ...
    (tmp_vault / "system" / "queue" / "b.json").write_text(json.dumps(
        {"id": "b", "skill": "ai-wire", "args": {}, "ts": "t3", "source": "voice"}
    ))
    _write_run(tmp_vault, "a", "metrics-pull", "api", "t0", "t1", "t2", "ok", 0, "done a", "system/runs/a.md", "inbox/a.md")
    _write_run(tmp_vault, "c", "metrics-pull", "api", "t4", "t5", "t6", "error", 1, "boom c", "system/runs/c.md", None)
